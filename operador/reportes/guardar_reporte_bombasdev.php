                ...
                if (!empty($obs['imagenes']) && is_array($obs['imagenes'])) {
                    ...
                    foreach ($obs['imagenes'] as $imgPath) {
                        ...
                        if (file_exists($realPath)) {
                            ...
                            // Si es la primera imagen de la fila, guardar Y
                            if ($count % 2 == 0) {
                                $rowStartY = $pdf->GetY();
                                $maxHeightInRow = 0;
                            }
                            ...
                            // Dibujar imagen SIN MARCO (sin border)
                            $pdf->Image($correctedPath, $x, $y, $w_mm, $h_mm);

                            // Trackear altura máxima en esta fila
                            $maxHeightInRow = max($maxHeightInRow, $h_mm);

                            // Si es la segunda imagen, hacer salto de línea
                            if ($count % 2 == 1) {
                    ...
                    // Si quedó una sola imagen sin pareja, bajar igual
                    if ($count % 2 == 1) {
                        $pdf->SetY($rowStartY + $maxHeightInRow + 8);
                    }
                }
