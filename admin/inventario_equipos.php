    ...
    if (isset($_POST['accion'])) {
        ...
        if ($accion === 'agregar') {
            ...
            $stmt->execute([
                ...
                $_POST['equipo_asociado'] ?: null, $_POST['estatus'],
                $_POST['planilla_especificaciones'], $_POST['fecha_validacion']
            ]);
        }

        if ($accion === 'editar') {
            $sql = "UPDATE equipos SET 
                        nombre=?, descripcion=?, identificador=?, colaborador=?, cliente=?, 
                        categoria=?, equipo_asociado=?, estatus=?, planilla_especificaciones=?, fecha_validacion=? 
                    WHERE id_equipo=?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                $_POST['nombre'], $_POST['descripcion'], $_POST['identificador'],
